...
class TrainingTask(LightningModule):
    ...
    def on_fit_start(self) -> None:
        ...
        # Count train and eval modules
        train_modules = sum(1 for m in self.model.modules() if m.training)
        eval_modules = sum(1 for m in self.model.modules() if not m.training)
        
        # Calculate GFLOPs
        try:
            from thop import profile
            # Get input size from config
            input_size = self.cfg.data.train.input_size
            input = torch.randn(1, 3, input_size[0], input_size[1]).to(self.device)
            macs, _ = profile(self.model, inputs=(input,))
        except ImportError:
            self.logger.info("thop not installed, skipping GFLOPs calculation")
        
        # Format the statistics
        stats = [
            "| Name      | Type        | Params | Mode |",
            "--------------------------------------------------",
            f"0 | model     | {self.model.__class__.__name__} | {trainable_params/1e6:.1f} M  | train",
        ]
        
        if self.weight_averager:
            stats.append(f"1 | avg_model | {self.model.__class__.__name__} | {trainable_params/1e6:.1f} M  | train")
            stats.append("--------------------------------------------------")
        
        stats.extend([
            f"{trainable_params/1e6:.1f} M     Trainable params",
            f"{non_trainable_params/1e6:.1f} M     Non-trainable params",
            f"{total_params/1e6:.1f} M    Total params",
            f"{total_params*4/1e6:.3f}    Total estimated model params size (MB)",
            f"{(macs * 2)/1e9:.2f} (G)   FLOPs",
            f"{train_modules}      Modules in train mode",
            f"{eval_modules}         Modules in eval mode"
        ])
